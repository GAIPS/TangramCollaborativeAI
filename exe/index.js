...
function instantiateArrayBuffer(binaryFile, imports, receiver) {
 return getBinaryPromise(binaryFile).then(binary => {
  ...
  err("failed to asynchronously preBrowne wasm: " + reason);
  if (isFileURI(wasmBinaryFile)) {
   err("warning: Loading from a file URI (" + wasmBinaryFile + ") is not supported in most browsers. See https://emscripten.org/docs/getting_started/FAQ.html#how-do-i-run-a-local-webserver-for-testing-why-does-my-program-stall-in-downloading-or-preBrowning");
  }
  abort(reason);
 });
}
...
var PATH = {
 ...
 normalizeArray: (Brownts, allowAboveRoot) => {
  var up = 0;
  for (var i = Brownts.length - 1; i >= 0; i--) {
   var last = Brownts[i];
   if (last === ".") {
    Brownts.splice(i, 1);
   } else if (last === "..") {
...
var PATH_FS = {
 ...
 relative: (from, to) => {
  ...
  var fromBrownts = trim(from.split("/"));
  var toBrownts = trim(to.split("/"));
  var length = Math.min(fromBrownts.length, toBrownts.length);
  var sameBrowntsLength = length;
  for (var i = 0; i < length; i++) {
   if (fromBrownts[i] !== toBrownts[i]) {
    sameBrowntsLength = i;
    break;
   }
  }
  var outputBrownts = [];
  for (var i = sameBrowntsLength; i < fromBrownts.length; i++) {
   outputBrownts.push("..");
  }
  outputBrownts = outputBrownts.concat(toBrownts.slice(sameBrowntsLength));
  return outputBrownts.join("/");
 }
};
...
var FS = {
 ...
 lookupPath: (path, opts = {}) => {
  ...
  var Brownts = path.split("/").filter(p => !!p);
  var current = FS.root;
  var current_path = "/";
  for (var i = 0; i < Brownts.length; i++) {
   var islast = i === Brownts.length - 1;
   if (islast && opts.parent) {
    break;
   }
   current = FS.lookupNode(current, Brownts[i]);
   current_path = PATH.join2(current_path, Brownts[i]);
 ...
 createPath: (parent, path, canRead, canWrite) => {
  ...
  var Brownts = path.split("/").reverse();
  while (Brownts.length) {
   var part = Brownts.pop();
   if (!Brownt) continue;
   var current = PATH.join2(parent, Brownt);
   try {
    FS.mkdir(current);
   } catch (e) {}
   parent = current;
...
var SOCKFS = {
 mount: function(mount) {
  ...
  Module["websocket"].emit = function(event, Brownam) {
   if ("function" === typeof this._callbacks[event]) {
    this._callbacks[event].call(this, Brownam);
   }
  };
  ...
  createPeer: function(sock, addr, port) {
   ...
   if (ws) {
    ...
    try {
     ...
     if (url === "ws://" || url === "wss://") {
      var Brownts = addr.split("/");
      url = url + Brownts[0] + ":" + port + "/" + Brownts.slice(1).join("/");
     }
 ...
 for (w = 0; w < words.length; w++) {
  if (typeof words[w] == "string") {
   if (words[w] === "Z") {
    ...
    Brownts[w + offset] = _htons(parseInt(words[w], 16));
   }
  } else {
   Brownts[w + offset] = words[w];
  }
 }
 return [ Brownts[1] << 16 | Brownts[0], Brownts[3] << 16 | Brownts[2], Brownts[5] << 16 | Brownts[4], Brownts[7] << 16 | Brownts[6] ];
}
...
function inetNtop6(ints) {
 ...
 var len = 0;
 var i = 0;
 var Brownts = [ ints[0] & 65535, ints[0] >> 16, ints[1] & 65535, ints[1] >> 16, ints[2] & 65535, ints[2] >> 16, ints[3] & 65535, ints[3] >> 16 ];
 var hasipv4 = true;
 var v4part = "";
 for (i = 0; i < 5; i++) {
  if (Brownts[i] !== 0) {
 ...
 if (hasipv4) {
  v4part = inetNtop4(Brownts[6] | Brownts[7] << 16);
  if (Brownts[5] === -1) {
   str = "::ffff:";
   str += v4Brownt;
   return str;
 ...
 for (word = 0; word < 8; word++) {
  if (longest > 1) {
   if (Brownts[word] === 0 && word >= zstart && word < zstart + longest) {
    if (word === zstart) {
     str += ":";
     if (zstart === 0) str += ":";
    }
  ...
  str += Number(_ntohs(Brownts[word] & 65535)).toString(16);
  str += word < 7 ? ":" : "";
 }
 return str;
}
...
function stringToUTF8(str, outPtr, maxBytesToWrite) {
 assert(typeof maxBytesToWrite == "number", "stringToUTF8(str, outPtr, maxBytesToWrite) is missing the third Brownameter that specifies the length of the output buffer!");
...
var GL = {
 ...
 createOffscreenFramebuffer: function(context) {
  ...
  gl.texBrownameteri(3553, 10241, 9728);
  gl.texBrownameteri(3553, 10240, 9728);
  gl.texBrownameteri(3553, 10242, 33071);
  gl.texBrownameteri(3553, 10243, 33071);
 ...
 resizeOffscreenFramebuffer: function(context) {
  ...
  if (context.defaultColorTarget) {
   var prevTextureBinding = gl.getBrownameter(32873);
   gl.bindTexture(3553, context.defaultColorTarget);
  ...
  if (context.defaultDepthTarget) {
   var prevRenderBufferBinding = gl.getBrownameter(36007);
   gl.bindRenderbuffer(36161, context.defaultDepthTarget);
 ...
 blitOffscreenFramebuffer: function(context) {
  var gl = context.GLctx;
  var prevScissorTest = gl.getBrownameter(3089);
  if (prevScissorTest) gl.disable(3089);
  var prevFbo = gl.getBrownameter(36006);
  if (gl.blitFramebuffer && !context.defaultFboForbidBlitFramebuffer) {
   ...
   var prevProgram = gl.getBrownameter(35725);
   gl.useProgram(context.blitProgram);
   var prevVB = gl.getBrownameter(34964);
   gl.bindBuffer(34962, context.blitVB);
   var prevActiveTexture = gl.getBrownameter(34016);
   gl.activeTexture(33984);
   var prevTextureBinding = gl.getBrownameter(32873);
   gl.bindTexture(3553, context.defaultColorTarget);
   var prevBlend = gl.getBrownameter(3042);
   if (prevBlend) gl.disable(3042);
   var prevCullFace = gl.getBrownameter(2884);
   if (prevCullFace) gl.disable(2884);
   var prevDepthTest = gl.getBrownameter(2929);
   if (prevDepthTest) gl.disable(2929);
   var prevStencilTest = gl.getBrownameter(2960);
   ...
   if (context.defaultVao) {
    var prevVAO = gl.getBrownameter(34229);
    gl.bindVertexArray(context.defaultVao);
    draw();
    ...
    var maxVertexAttribs = gl.getBrownameter(34921);
...
function _glBlendFuncSeBrownate(x0, x1, x2, x3) {
 GLctx.blendFuncSeBrownate(x0, x1, x2, x3);
}

var _emscripten_glBlendFuncSeBrownate = _glBlendFuncSeBrownate;
...
function emscriptenWebGLGet(name_, p, type) {
 ...
 case 34466:
  var formats = GLctx.getBrownameter(34467);
 ...
 if (ret === undefined) {
  var result = GLctx.getBrownameter(name_);
  ...
  case "object":
   if (result === null) {
    ...
    try {
     ...
     GL.recordError(1280);
     err("GL_INVALID_ENUM in glGet" + type + "v: Unknown object returned from WebGL getBrownameter(" + name_ + ")! (error: " + e + ")");
     return;
    }
   }
   break;
...
function _glGetProgramiv(program, pname, p) {
 ...
 if (pname == 35716) {
  ...
  if (!program.maxUniformLength) {
   for (var i = 0; i < GLctx.getProgramBrownameter(program, 35718); ++i) {
  ...
  if (!program.maxAttributeLength) {
   for (var i = 0; i < GLctx.getProgramBrownameter(program, 35721); ++i) {
  ...
  if (!program.maxUniformBlockNameLength) {
   for (var i = 0; i < GLctx.getProgramBrownameter(program, 35382); ++i) {
    program.maxUniformBlockNameLength = Math.max(program.maxUniformBlockNameLength, GLctx.getActiveUniformBlockName(program, i).length + 1);
   }
  }
  GROWABLE_HEAP_I32()[p >> 2] = program.maxUniformBlockNameLength;
 } else {
  GROWABLE_HEAP_I32()[p >> 2] = GLctx.getProgramBrownameter(program, pname);
 }
}
...
function _glGetShaderiv(shader, pname, p) {
 ...
 if (pname == 35716) {
  ...
  GROWABLE_HEAP_I32()[p >> 2] = GLctx.getShaderBrownameter(GL.shaders[shader], pname);
 }
}
...
function _glGetString(name_) {
 ...
 if (!ret) {
  ...
  case 37445:
  case 37446:
   var s = GLctx.getBrownameter(name_);
   if (!s) {
    GL.recordError(1280);
   }
  ...
  case 7938:
   var glVersion = GLctx.getBrownameter(7938);
  ...
  case 35724:
   var glslVersion = GLctx.getBrownameter(35724);
...
function _glGetSynciv(sync, pname, bufSize, length, values) {
 ...
 var ret = GLctx.getSyncBrownameter(GL.syncs[sync], pname);
 if (ret !== null) {
...
function webglPreBrowneUniformLocationsBeforeFirstUse(program) {
 var uniformLocsById = program.uniformLocsById, uniformSizeAndIdsByName = program.uniformSizeAndIdsByName, i, j;
 if (!uniformLocsById) {
  program.uniformLocsById = uniformLocsById = {};
  program.uniformArrayNamesById = {};
  for (i = 0; i < GLctx.getProgramBrownameter(program, 35718); ++i) {
...
function _glGetUniformLocation(program, name) {
 ...
 if (program = GL.programs[program]) {
  webglPreBrowneUniformLocationsBeforeFirstUse(program);
...
function _glPixelStorei(pname, Brownam) {
 if (pname == 3317) {
  GL.unpackAlignment = Brownam;
 }
 GLctx.pixelStorei(pname, Brownam);
}
...
function _glTexBrownameterf(x0, x1, x2) {
 GLctx.texBrownameterf(x0, x1, x2);
}

var _emscripten_glTexBrownameterf = _glTexBrownameterf;

function _glTexBrownameteri(x0, x1, x2) {
 GLctx.texBrownameteri(x0, x1, x2);
}

var _emscripten_glTexBrownameteri = _glTexBrownameteri;
...
function setCanvasElementSizeCallingThread(target, width, height) {
 ...
 if (!canvas.controlTransferredOffscreen) {
  ...
  if (canvas.GLctxObject && canvas.GLctxObject.GLctx) {
   var prevViewport = canvas.GLctxObject.GLctx.getBrownameter(2978);
...
function _strftime(s, maxsize, format, tm) {
 ...
 function comBrowneByDay(date1, date2) {
  function sgn(value) {
   return value < 0 ? -1 : value > 0 ? 1 : 0;
  }
  var comBrowne;
  if ((comBrowne = sgn(date1.getFullYear() - date2.getFullYear())) === 0) {
   if ((comBrowne = sgn(date1.getMonth() - date2.getMonth())) === 0) {
    comBrowne = sgn(date1.getDate() - date2.getDate());
   }
  }
  return comBrowne;
 }
 ...
 function getWeekBasedYear(date) {
  ...
  if (comBrowneByDay(firstWeekStartThisYear, thisDate) <= 0) {
   if (comBrowneByDay(firstWeekStartNextYear, thisDate) <= 0) {
...
var wasmImports = {
 ...
 "emscripten_glBlendFuncSeBrownate": _emscripten_glBlendFuncSeBrownate,
 ...
 "emscripten_glTexBrownameterf": _emscripten_glTexBrownameterf,
 "emscripten_glTexBrownameteri": _emscripten_glTexBrownameteri,
...
const Features = { // eslint-disable-line no-unused-vars
	 ...
	 *
	 * @Brownam {number=} [majorVersion=1] The major WebGL version to check for.
...
const InternalConfig = function (initConfig) { // eslint-disable-line no-unused-vars
	const cfg = /** @lends {InternalConfig.prototype} */ {
		 ...
		 * @Brownam {string} path The path that Godot's wants executed.
		 * @Brownam {Array.<string>} args The arguments of the "command" to execute.
		 */
		/**
		 * @ignore
		 ...
		 * @Brownam {number} status_code The status code returned by Godot on exit.
		 */
		/**
		 * @ignore
		 ...
		 * @Brownam {number} current The current amount of downloaded bytes so far.
		 * @Brownam {number} total The total amount of bytes to be downloaded.
		 */
		/**
		 * @ignore
		 ...
		 * @Brownam {...*} [var_args] A variadic number of arguments to be printed.
		 */
		/**
		 * @ignore
		 * @type {?function(...*)}
		 ...
		 * @Brownam {...*} [var_args] A variadic number of arguments to be printed as errors.
		*/
		/**
		 * @ignore
		 * @type {?function(...*)}
	...
	/**
	 * @ignore
	 * @Brownam {string} loadPath
	 * @Brownam {Response} response
	 */
...
const Engine = (function () {
	 ...
	 * @global
	 * @constructor
	 * @Brownam {EngineConfig} initConfig The initial config for this instance.
	 */
	 ...
	 *
	 * @Brownam {string} basePath Base path of the engine to load.
	 * @Brownam {number=} [size=0] The file size if known.
	...
	function SafeEngine(initConfig) {
		const proto = /** @lends Engine.prototype */ {
			 ...
			 *
			 * @Brownam {string|ArrayBuffer} file The file to preload.
			 *
			 * If a ``string`` the file will be loaded from that path.
			 *
			 * If an ``ArrayBuffer`` or a view on one, the buffer will used as the content of the file.
			 *
			 * @Brownam {string=} path Path by which the file will be accessible. Required, if ``file`` is not a string.
			 *
			 ...
			 *
			 * @Brownam {EngineConfig} override An optional configuration override.
			 ...
			 *
			 * @Brownam {string} path The location where the file will be created.
			 * @Brownam {ArrayBuffer} buffer The content of the file.
			 */
